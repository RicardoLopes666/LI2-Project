...
void mostrarTabela(TABELA t)
{
    ...
    printf("    ");
    for (int col = 0; col < t->c; col++)
    {
        printf(" %c  ", 'a' + col);
    }
    printf("\n");

    // Linha de topo da grelha
    printf("   ╔");
    for (int col = 0; col < t->c; col++)
    {
        printf("═══");
    ...
    for (int linha = 0; linha < t->l; linha++)
    {
        // Número da linha
        printf("%2d ║", linha + 1);
        ...
        for (int col = 0; col < t->c; col++)
        {
            char c = t->tabela[linha][col];

            if (c >= 'a' && c <= 'z')
            {
                // Letras minúsculas (pretas)
                printf("\033[1;30m %c \033[0m", c);
            }
            else if (c == '#')
            {
                // Casa riscada (vermelho)
                printf("\033[1;31m # \033[0m");
            }
            else if (c >= 'A' && c <= 'Z')
            {
                // Letras maiúsculas (brancas)
                printf("\033[1;37m %c \033[0m", c);
            }
            else
            {
                // Qualquer outro carácter (vazio, espaço, etc.)
                printf(" %c ", c);
            }
