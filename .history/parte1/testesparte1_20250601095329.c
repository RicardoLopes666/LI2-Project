...
void test_lerCmd(void)
{
    ...
    CU_ASSERT_PTR_NOT_NULL_FATAL(game.stackTabs);
    CU_ASSERT_TRUE_FATAL(initStackTabs(game.stackTabs));
    CU_ASSERT_FALSE(lerCmd('l', "nonexistent.txt", &game));
    freeStackTabs(game.stackTabs);

    // Formato de dimensões inválido (não inteiros)
    game.estado.looping = true;
    game.tab = NULL;
    game.stackTabs = malloc(sizeof(struct StackTabs));
    CU_ASSERT_PTR_NOT_NULL_FATAL(game.stackTabs);
    ...
    f = fopen(filename, "w");
    CU_ASSERT_PTR_NOT_NULL_FATAL(f);
    fprintf(f, "2 2\nA\n"); // Apenas uma linha de dados em vez de 2
    fclose(f);
    CU_ASSERT_FALSE(lerCmd('l', filename, &game));
    // A stack é libertada dentro de lerCmd em caso de erro
    remove(filename);
}
...
void test_e(void)
{
    ...
    // Cria tabuleiro 2x2 e solução 2x2
    TABELA t = malloc(sizeof(struct Tabela));
    TABELA s = malloc(sizeof(struct Tabela));
    initTabela(t, 2, 2);
    initTabela(s, 2, 2);

    // Preenche t com: 'A' (correta), 'b' (minúscula), '#' (riscada), e 'C' incorreta
    t->tabela[0][0] = 'A';
    t->tabela[0][1] = 'b';
    t->tabela[1][0] = '#';
    t->tabela[1][1] = 'D';

    // Solução apenas letras maiúsculas corretas em todas as posições
    s->tabela[0][0] = 'A';
    s->tabela[0][1] = 'B';
    s->tabela[1][0] = 'C';
    s->tabela[1][1] = 'D';
    ...
    // Chama e(): não validamos a saída impressa, apenas a flag
...
// Função main para registrar e rodar todos os testes
int main()
{
    CU_initialize_registry();

    CU_pSuite suite = CU_add_suite("Teste Parte1", NULL, NULL);
    ...
    CU_add_test(suite, "test_lerCmd", test_lerCmd);
    CU_add_test(suite, "test_i", test_i);
    CU_add_test(suite, "test_e", test_e);
