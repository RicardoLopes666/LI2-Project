...
void mostrarTabela(GAME *game)
{
    ...
    for (int i = 0; i < t->l; i++)
    {
        ...
        for (int j = 0; j < t->c; j++)
        {
            ...
            int is_last = (i == game->last_move_l && j == game->last_move_c);
            const char *effect = is_last ? REVERSE : "";
            const char *end_effect = is_last ? RESET : "";
            if (cell == '#')
                printf("%s%s # %s%s│", effect, HASHED_CELL, end_effect, RESET);
            else if (isupper(cell))
                printf("%s%s[%c]%s%s│", effect, CAPITAL_LETTER, cell, end_effect, RESET);
            else
                printf("%s %s%c%s %s│", effect, NORMAL_LETTER, cell, end_effect, RESET);
        }
        printf("\n");
        if (i < t->l - 1)
        {
