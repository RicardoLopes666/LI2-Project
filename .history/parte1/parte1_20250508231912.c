...
void mostrarTabela(GAME *game)
{
    ...
    printf("    │");
    for (int j = 0; j < t->c; j++)
        printf(" %s%c%s │", COR_COORDENADA, 'a' + j, REINICIAR);
    printf("\n");
    ...
    for (int i = 0; i < t->l; i++)
    {
        printf("%s%2d%s │", COR_COORDENADA, i + 1, REINICIAR);
        for (int j = 0; j < t->c; j++)
        {
            char cell = t->tabela[i][j];
            if (cell == '#')
                printf("%s # %s│", CELULA_RISCADA, REINICIAR);
            else if (isupper(cell))
                printf("%s[%c]%s│", LETRA_MAIUSCULA, cell, REINICIAR);
            else
                printf(" %s%c%s │", LETRA_NORMAL, cell, REINICIAR);
        }
        printf("\n");
        if (i < t->l - 1)
        {
