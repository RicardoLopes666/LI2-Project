...
void trataComandoaAR(char comando, char *arg, GAME *game, bool *continuar, bool *comandoProcessado)
{
    if (arg[0] != '\0')
    {
        *continuar = false;
    ...
    else
    {
        ...
        {
            printf("%sDe volta ao tabuleiro inicial%s", )
        }
    }
}
...
void comandovduc(bool *continuar, char comando, char *arg, GAME *game, bool *comandoProcessado)
{
    if (comando == 'v' || comando == 'd' || comando == 'u')
    {
        if (arg[0] != '\0')
        {
            *continuar = false;
    ...
    while (game.estado.looping)
    {
        ...
        if (comandoQAlteraTab(cmd[0], continuar, comandoProcessado))
        {
            if (cmd[0] == 'b' || cmd[0] == 'r')
            {
                ...
                trataComandoaARi(cmd[0], arg, &game, &continuar, &comandoProcessado); // Função que trata dos comando 'a', 'A' e 'R'
        ...
        comandovduc(&continuar, cmd[0], arg, &game, &comandoProcessado);
    }
