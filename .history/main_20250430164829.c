...
int main()
{
    ...
    while (game.estado.looping)
    {
        ...
        if (!comandoProcessado && continuar)
        {
            ...
            if (cmd[0] == 'b' || cmd[0] == 'r' || cmd[0] == 'a' || cmd[0] == 'R' || cmd[0] == 'A')
            {
                if (cmd[0] == 'b' || cmd[0] == 'r')
                {
                    if (arg[0] == '\0')
                ...
                else if (cmd[0] == 'R' && continuar)
                {
                    printf("A executar o comando de resolver o jogo...\n");
                    TABELA aux = resolve(game.stackTabs->tabelas[0], game.tab); // Recebe a primeira tabela carregada (em principio so com letras minusculas)
                    if (aux != NULL)
                    {
                        freeTabela(game.tab);
                        game.tab = aux;
