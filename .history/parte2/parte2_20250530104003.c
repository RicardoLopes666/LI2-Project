...
// Código para o comando c-> o comando c vai ao estado atual do tabuleiro e indica se as casas que foram alteradas estão corretas
void c(GAME game)
{
    for (int i = 0; i < game.tab->l; i++)
    {
        ...
        {
            if (islower(game.tab->tabela[i][j]))
            {
            }
        }
    }
