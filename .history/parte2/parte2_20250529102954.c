...
{                                                                          // Apenas se reduz o indice caso possivel
    if (game->stackTabs->comprimento <= 1 || game->stackTabs->indice <= 1) // Dá erro se tentar eliminar a tabela inicial
        return false;

    game->tab = copiarTabela(game->stackTabs->tabelas[game->stackTabs->indice-- - 1]);
    return true;
}
